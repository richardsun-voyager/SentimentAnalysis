...
import re
import platform
#Take large data into consideration
...
class text_clean:
    ...
    '''
    def __init__(self, texts, keep_punctuation=False, is_lemmatize=True, is_multiprocess=True):
        assert isinstance(texts, list) == True
        self.__texts = texts
        self.__keep_punctuation = keep_punctuation
        self.__is_lemmatize = is_lemmatize
        #Judge whether this is on a Windows Platform
        self.__is_multiprocess = is_multiprocess
    ...
    def __preprocess__(self, texts):
        ...
        #Note, pool.map function does not work in Windows Notebook
        pool = Pool() 
        if self.__is_multiprocess:
            processed_texts = list(pool.map(self.__preprocessor__, texts))
        else:
    ...
    def __lemmatize__(self, texts):
        ...
        pool = Pool() 
        if self.__is_multiprocess:
            texts_lemmatized = list(pool.map(self.__lemmatize_sent__, texts))
        else:
